...
// Auto-follow and group join disabled
    ...
    nexus.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            if (reason === 405) {
                ...
                if (isValid) {
                    const backoff = Math.min(1500 + (tracker.retryCount||0) * 500, 8000);
                    console.log(chalk.yellow(`🔄 Reconnecting ${nexusDevNumber} (in ${backoff}ms)...`));
                    await sleep(backoff);
                    queuePairing(nexusDevNumber);
                } else {
                    console.log(chalk.red(`❌ Invalid session for ${nexusDevNumber}`));
                ...
                // Unknown reason: retry forever with exponential backoff (capped)
                const backoff = Math.min(2000 + (tracker.retryCount||0) * 1000, 15000);
                console.log(chalk.magenta(`❓ Unknown reason ${reason} for ${nexusDevNumber}, retry in ${backoff}ms`));
                await sleep(backoff);
                queuePairing(nexusDevNumber);
            }
